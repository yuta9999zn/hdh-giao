...
module gvm #(parameter MEM = 4096, parameter WORD = 16) (
  ...
  localparam S_FETCH=5'd0, S_EXEC=5'd1, S_STR=5'd2, S_CLO=5'd3, S_DS=5'd4, S_DIV=5'd5,
             ...
             S_GHITRUONG=5'd22;  // ★ SIÊU-LỆNH #3: S_EXEC đọc ram[rel_o]→ram_q; S_GHITRUONG ghi ram[rel_s1]=ram_q (RAM-to-RAM 2 chu-kỳ)
  ...
  wire [15:0] ram_ra = (state==S_EXEC && opcode==OP_TAI_O)       ? rel_o
  ...
  initial $readmemh("program_ngat.hex", rom);
      ...
      if (state == S_FETCH) begin
        ...
        case (opcode)
          ...
          OP_LUU_O:   begin if (viol_o) begin out_data<=16'd911; out_valid<=1; halt<=1; end
                            else begin ram[rel_o] <= opB; sp <= sp - 8'd1; end end   // LƯU_Ô: ghi opB(=stack[sp-1])
          ...
          OP_LUU_GIAN:begin if (viol_s2) begin out_data<=16'd911; out_valid<=1; halt<=1; end
                            else begin ram[rel_s2] <= opB; sp <= sp - 8'd2; end end // relocated (ghi opB=stack[sp-1])
        ...
        ram[rel_s1] <= ram_q; state <= S_FETCH;   // ★ #3 store-field: ghi ram[top=rel_s1] = ram_q(=ram[r]). sp GIỮ (rel_s1 dùng opB còn-giữ).
